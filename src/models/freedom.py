...
from src.utils.utils import build_sim, compute_normalized_laplacian
...
class FREEDOM(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.mamba_layers_r = nn.ModuleList([
            MambaLayer(
                d_model=self.hidden_size,
                d_state=self.d_state,
                d_conv=self.d_conv,
    ...
    def pre_epoch_processing(self):
        ...
        degree_len = int(self.edge_values.size(0) * (1. - self.dropout1))
    ...
    def calculate_loss(self, interaction):
        ...
        if self.t_feat is not None:
            ...
            text_feats = torch.squeeze(text_feats, 0)
        ...
        if self.v_feat is not None:
            ...
            image_feats = torch.squeeze(image_feats, 0)
...
class MambaLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        if self.num_layers == 1:  # one Mamba layer without residual connection
            ...
            hidden_states = self.LayerNorm1(self.dropout(hidden_states) + input_tensor)
            hidden_states = self.LayerNorm2(self.dropout(hidden_states) + input_tensor)
...
class FeedForward(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        hidden_states = self.dropout(hidden_states)
        # hidden_states = self.RMSNnorm(hidden_states + input_tensor)
